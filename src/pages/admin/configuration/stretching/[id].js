...
export default function EditStretching(props) {
  ...
  const [description, setDescription, descriptionBinds] = useInput(null);
  ...
  useEffect(() => {
    if (!isCreate && id) {
      stretchingService.get(id).then(setStretching);
    }
  }, [id]);
  ...
  useEffect(() => {
    ...
    setDisplayName(stretching.displayName);
    setDescription(stretching.description);
    setPriceCode(stretching.priceCode);
    setIsNew(stretching.isNew);
    setIsDeleted(stretching.isDeleted);
  }, [stretching]);
  ...
  const onSubmit = createSubmitHandler(
    ...
    (obj) => obj.displayName,
    (obj) => `/admin/configuration/stretching/${obj.id}`,
    !isCreate ? id : null,
    isCreate,
    setStretching,
  );
  ...
  return (
    <AdminPage title="Edit stretching">
      <Form onSubmit={onSubmit}>
        ...
        <button type="button" onClick={() => router.back()} className="btn btn-lg btn-primary mt-3 ml-3">Back to Stretching</button>
      </Form>
    </AdminPage>
  );
}
