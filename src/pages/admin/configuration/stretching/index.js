...
const columns = [
  ...
  },
  {
    ...
    formatter: (cell, row) => {
      return (
        <Link href={`/admin/configuration/stretching/${row.id}`} as={`/admin/configuration/stretching/${row.id}`}>{row.displayName}</Link>
      );
    },
  },
  {
...
export default function Stretching(props) {
  ...
  useEffect(() => {
    (async () => {
      const results = await stretchingService.find({includeDeleted: true});
      ...
      setStretchings(results);
    })();

  }, []);

  return (
    <AdminPage title="Stretching">
      <ToolkitProvider
        keyField="id"
        data={ stretchings }
        columns={ columns }
      ...
      >
        {
          props => (
            <div>
              ...
              />
              <div>
                <div className="btn btn-primary mt-2" onClick={(e) => router.push("/admin/configuration/stretching/new")}>Create</div>
              </div>
            </div>
          )
        }
