...
const columns = [
  ...
  },
  {
    ...
    formatter: (cell, row) => {
      return (
        <Link href={`/admin/configuration/edgewidth/${row.id}`} as={`/admin/configuration/edgewidth/${row.id}`}>{row.displayName}</Link>
      );
    },
  },
  {
...
export default function EdgeWidth(props) {
  ...
  useEffect(() => {
    (async () => {
      const results = await edgeWidthService.find({ includeDeleted: true });
      ...
      setEdgeWidths(results);
    })();

  }, []);

  return (
    <AdminPage title="Edge Width">
      <ToolkitProvider
        keyField="id"
        data={ edgeWidths }
        columns={ columns }
      ...
      >
        {
          props => (
            <div>
              ...
              />
              <div>
                <div className="btn btn-primary mt-2" onClick={(e) => router.push("/admin/configuration/edgewidth/new")}>Create</div>
              </div>
            </div>
          )
        }
